    ...
    for root, dirs, files in os.walk(root_choose):
        ...
        for jav in list_jav_struct:
            ...
            if jav.subtitle:
                ...
                # 针对有多集的影片做特殊处理，部分影片的命名规则中，会使用 "-C" 代表第三集
                tmp_name = jav.car
                if dict_car_pref[jav.car] > 2 and jav.episode == 3:
                    tmp_name = tmp_name.replace('-C', '').replace('_C', '').replace('-c', '').replace('_c', '')
            ...
            try:
                ...
                try:
                    ...
                    #                                root_classify)
                    jav, num_temp = classify_link_files(jav, num_fail, settings, dict_data, list_classify_basis,
                                                   root_classify, list_name_video, str_cd)
                    num_fail = num_temp
                except FileExistsError:
                    num_fail += 1
                    continue
