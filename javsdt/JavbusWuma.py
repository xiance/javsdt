...
from Functions.Car import find_car_wuma, list_suren_car
from Functions.Standard import collect_sculpture
from Functions.Picture import crop_poster_baidu, crop_poster_default
...
list_surplus_words_in_filename = settings.list_surplus_word_in_filename('无码')
...
# 优化特征的字典
dict_genre = better_dict_genre('Javbus无码', to_language)
    ...
    for root, dirs, files in os.walk(root_choose):
        ...
        for file_raw in files:
            ...
            if file_temp.endswith(('.SRT', '.VTT', '.ASS', '.SSA', '.SUB', '.SMI',)):
                ...
                # 得到字幕文件名中的车牌
                subtitle_car = find_car_wuma(file_temp, list_suren_cars)
        ...
        for file_raw in files:
            ...
            if file_temp.endswith(tuple_video_types) and not file_temp.startswith('.'):
                ...
                for word in list_surplus_words_in_filename:
                    file_temp = file_temp.replace(word, '')
                # 得到视频中的车牌
                car = find_car_wuma(file_temp, list_suren_cars)
                if car:
                    try:
        ...
        for jav in list_jav_struct:
            ...
            try:
                ...
                # 用户没有指定网址，则去搜索
                    ...
                    url_search_web = url_bus + 'uncensored/search/' + jav.car.replace('-', '%20').replace('_',
                                                                                                          '%20').replace(
                        ' ', '%20') + '&type=&parent=uc'
                    ...
                    if list_search_results:  # 搜索结果页面只有一个box
                        ...
                        # print('    >正在核查搜索结果...')
                        jav_prefg = re.search(r'([A-Z0-9]+)[-_]?', jav.car)
                        jav_pref = jav_prefg.group(1) if str(jav_prefg) != 'None' else ''
                        jav_sufg = re.search(r'[^\d](\d\d+)', jav.car)
                        ...
                        for i in list_search_results:
                            ...
                            url_sufg = re.search(r'[^\d](\d\d+)', url_end)
                            url_suf = url_sufg.group(1).lstrip('0') if str(
                                url_sufg) != 'None' else ''  # 匹配box上影片url，车牌的后缀数字，去除多余的0
                            # print(url_end, url_suf)
                            if jav_suf == url_suf:  # 数字相同
                                url_prefg = re.search(r'([A-Z0-9]+)[-_]?', url_end)
                        ...
                        if not list_fit_results:
                            num_fail += 1
                            record_fail('    >第' + str(
                                num_fail) + '个失败！javbus无码找不到该车牌的信息：' + jav.car + '，' + path_relative + '\n')
                            continue  # 【退出对该jav的整理】
                        # 默认用第一个搜索结果
                ...
                if actors:
                    ...
                    actors = ['有码演员']
                    dict_data['首个演员'] = dict_data['全部演员'] = '无码演员'
                # 处理影片的标题过长
